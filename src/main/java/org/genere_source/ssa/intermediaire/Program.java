...
public class Program {
	...
	public boolean est_valide(List<String> liste_erreurs) {
		...
		for(int i=0;i<liste_variables.size();i++)
		{
			...
			nom_var=liste_variables.get(i).getNom();
			...
			for(int j=0;j<i;j++)
			{
				if(liste_variables.get(j).getNom().equalsIgnoreCase(nom_var))
				{
					liste_erreurs.add("La variable "+nom_var+" est d�clar�e plusieurs fois");
					return false;
				}
			}
		}
		...
		for(Instr ins:liste_instructions)
		{
			...
			else if(ins instanceof Affect)
			{
				...
				if(donne_var(tmp.getVar().getNom())==-1)
				{
					liste_erreurs.add("La variable "+tmp.getVar().getNom()+" n'existe pas (instr "+no+")");
					return false;
				}
				assert(tmp.getExpr()!=null);
				if(!verif_expr(liste_erreurs,tmp.getExpr(),no))
	...
	private boolean verif_expr(List<String> liste_erreurs, Expr expr,int no_instr) {
		...
		{
			Var tmp=(Var) expr;
			if(tmp.getNom()==null||tmp.getNom().isEmpty())
			{
				liste_erreurs.add("Le nom de la variable est invalide (instr "+no_instr+")");
				return false;
			}
			if(donne_var(tmp.getNom())==-1)
			{
				liste_erreurs.add("La variable "+tmp.getNom()+" n'existe pas (instr "+no_instr+")");
				return false;
			}
		}
	...
	private int donne_var(String nom) {
		...
		for(int i=0;i<liste_variables.size();i++)
		{
			if(liste_variables.get(i).getNom().equalsIgnoreCase(nom))
				return i;
		}
		return -1;
	}
	...
	public void calcul_basic_bloc()
	{
		...
		List<String> labels;
		liste_basic_bloc=new ArrayList<BasicBloc>();
		// on d�coupe les bloques
