...
public class SSA {
	...
	public SSA(Program program)
	{
		this.program=program;
		this.liste_basic_bloc=program.getListe_basic_bloc();
	}
	...
	public void convertie_ssa()
	{
		...
		calcul_dominance();
		for(Var v:program.getListe_variables())
		{
	...
	private void renome_variables() {
		...
		{
			System.out.println("****************************");
			System.out.println("prog="+program.toString());
			System.out.println("bloques de base="+program.BloctoString());
			pile=new ArrayList<Integer>();
			debut=liste_basic_bloc.get(0);
			for(Var x:program.getListe_variables())
			{
			...
			System.out.println("R�sultat="+program.BloctoString());
			
		}
	}

	private int renome_variables(Var x, BasicBloc b, List<Integer> pile, int v) {
		int ve;
		Var sommet;
		assert(x!=null);
		assert(pile!=null);
		...
		sommet=new Var(x.getNom()+pile.get(pile.size()-1),x.getType());
		for(Instr ins:b.getListe_instr())
		{
			...
			// on remplace les d�finitions de la variable
		...
		for(BasicBloc b:liste_basic_bloc)
		{
			for(Instr ins:b.getListe_instr())
			{
				if(ins instanceof Affect)
				{
					...
					Var n;
					n=tmp.getVar();
					if(n.getNom().equalsIgnoreCase(v.getNom()))
					{
		...
		while(!w.isEmpty())
		{
			...
			for(BasicBloc c:t.getDominance_frontiere())
			{
				if(!has_phi[c.getNo()])
				{
					...
					{
						liste.add(new Var(v.getNom(),v.getType()));
					}
					ins=new Phi(new Var(v.getNom(),v.getType()),liste);
